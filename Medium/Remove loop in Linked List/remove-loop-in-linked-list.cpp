...
class Solution
{
    ...
    Node*findstart(Node*head, Node*newhead){
        while(head!=newhead){
            head=head->next;
    ...
    Node* detect(Node*head){
        ...
        while(fast!=NULL && fast->next!=NULL){
            ...
            fast=fast->next->next;
            if(slow==fast){
                return fast; // Return the fast pointer when loop detected
            }
        }
        return NULL;
    }
    void removeLoop(Node* head)
    {
        ...
        Node* newhead=detect(head);
        if(newhead==NULL) return;
        Node*starting=findstart(head,newhead);
        Node*temp=starting;
